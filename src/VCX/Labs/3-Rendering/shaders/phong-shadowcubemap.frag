...
float Shadow(vec3 pos, vec3 lightPos) {
    ...
    float closestDepth = texture(u_ShadowCubeMap, normalize(toLight)).x * u_FarPlane;
    // your code end
...
vec3 Shade(vec3 lightIntensity, vec3 lightDir, vec3 normal, vec3 viewDir, vec3 diffuseColor, vec3 specularColor, float shininess) {
    // your code here:
    vec3 L_d = diffuseColor * lightIntensity * max(dot(lightDir, normal), 0.0);

    vec3 halfDir = normalize(lightDir + viewDir);
    vec3 L_s = lightIntensity * pow(max(dot(halfDir, normal), 0.0), shininess) * specularColor;
